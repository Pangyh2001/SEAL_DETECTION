...
class SealVerifier:
    ...
    @torch.no_grad()
    def verify_pair(self, scan_path, template_path, visualize=False):
        """
        验证单对印章
        ...
        Args:
            ...
            visualize: 是否可视化结果（作为后端服务时一般为False）
...
# 你可以根据实际情况改这个路径
MODEL_PATH_SERVICE = "outputs/seal_verification3/checkpoint_epoch_30.pth"

# 全局加载一次模型，给接口复用
verifier = SealVerifier(MODEL_PATH_SERVICE)

app = FastAPI(title="Seal Verification API", description="印章一致性验证接口")

# CORS，方便本地前端访问
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # 可以根据需要改成具体域名
...
@app.post("/api/compare_seal")
async def compare_seal(
    ...
    - image_a: 第一张印章图片
    - image_b: 第二张印章图片
    - threshold: 相似度阈值（可选，前端不传就用默认 0.5）
    返回：
    {
      "similarity": 0.92,
      "same": true,
    ...
    """
    # 把上传的文件写到临时文件，复用你原先按路径读取的逻辑
    tmp_a = tempfile.NamedTemporaryFile(delete=False, suffix=Path(image_a.filename).suffix)
    tmp_b = tempfile.NamedTemporaryFile(delete=False, suffix=Path(image_b.filename).suffix)
    try:
        tmp_a.write(await image_a.read())
        tmp_a.flush()
        tmp_b.write(await image_b.read())
        tmp_b.flush()

        scan_path = tmp_a.name
        template_path = tmp_b.name

        # 调用原来的推理逻辑（不画图）
        result = verifier.verify_pair(scan_path, template_path, visualize=False)

        sim = float(result["similarity"])
        thr = float(threshold)
        same = sim >= thr
        message = "模型认为为同一枚印章" if same else "模型认为不是同一枚印章"
        ...
        return {
            ...
            "inference_time": float(result["inference_time"]),  # 秒
            "message": message,
        }
    finally:
        # 删除临时文件
